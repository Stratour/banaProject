...
- **Renamed vs. genuinely removed — don't guess, check git history.** A 404'd legacy URL is either (a) renamed — a live equivalent still exists, redirect to it — or (b) the content was deliberately deleted with no successor — in which case there is nothing to redirect to, and manufacturing a redirect to a loosely-related page is worse than leaving it 404. Distinguish the two with `git log -p --follow -- <urls-file>` (or the app's routing file) around the suspect route name: if the view function/class still exists under a new path, it's a rename (redirect it); if the view itself was deleted from the codebase, it's a removal (leave it, or see next point). Never assume a redirect target — confirm it existed and ask the user if the git trail is ambiguous.
- **Permanently removed content: prefer 410 Gone over a bare 404** when the removal is confirmed deliberate and permanent (no successor page). A plain 404 gets periodically re-crawled by Google "in case it comes back"; a 410 tells it explicitly not to bother, which de-indexes the URL faster. Don't force this by default — a 404 is perfectly fine and requires no code change — but offer it as a faster-cleanup option when the user confirms the removal is final.
...
- **Static/media assets can silently escape the canonicalization redirect.** If the `www`/apex (or `http`/`https`) redirect is implemented in application middleware rather than at the reverse-proxy/webserver level, any path served directly by the webserver via `alias`/static file serving (typically `/static/`, `/media/`, uploads) never reaches the app and never gets redirected — even though every HTML page does. This produces genuine, real duplicate-host indexing (not the harmless kind) for PDFs, images, and other static files specifically. Test it explicitly: `curl -sI` the apex/non-canonical version of a static asset (not just an HTML page) and confirm it also 301s. If it doesn't, the fix belongs in the webserver config (nginx `server_name`-based redirect block, or a location-level rule), not in application code.
- **Non-HTML files (PDF, DOCX, images) can't carry a `<meta name="robots">` tag.** To noindex a PDF (common candidates: mentions légales / terms / privacy / cookies policy and other footer legal-utility documents — low search value, sometimes contain a named individual's address, occasionally indexed by default with no one having decided that on purpose), the only mechanism is the `X-Robots-Tag: noindex` HTTP response header, set at whichever layer actually serves the file (usually the webserver config for statically-served assets, since they bypass the app). When auditing a site's footer/legal documents, check whether these already carry this header — if not, flag it as a low-effort, generally-recommended fix, not a hard requirement (the file must stay *accessible*, e.g. for legal-disclosure obligations — noindex only removes it from search results, not from the page that links to it).
- **URL-prefix Search Console properties can under- or over-report depending on which host they're scoped to.** A property verified for `https://bana.mobi` (not `www`) cannot inspect a `www.bana.mobi` URL at all, and will show almost the entire site as "page with redirect" if the site's canonical host is actually `www` (or vice versa) — because the property is watching the *wrong* (redirecting) host, not because anything is broken. If a site's indexing report looks alarmingly bad but the canonical/redirect setup checks out fine, check which exact property type and host is configured before concluding there's a real problem. Recommend switching to (or adding) a **Domain property** (DNS TXT verification, covers `http`/`https`/`www`/non-`www` in one unified view) — it eliminates this whole class of false alarm and is the only property type that can inspect every host variant of the site.
...
> **Note (Claude Code environment):** this step was authored for Claude's Cowork/desktop file-generation environment (`/sessions/.../mnt/outputs/`, `present_files`, `computer://` links). None of those exist in Claude Code. In this environment, skip DOCX/PDF generation and instead deliver the equivalent structured findings directly in chat (or as a Markdown/Artifact file written to the repo/scratchpad), keeping the same sections and scoring below.
...
*Installed from https://github.com/SNLabat/SEO-GEO-AEO-Skill — audits live URLs and does not cover local image-file format/compression optimization (its only image signal is alt-text descriptiveness).*
